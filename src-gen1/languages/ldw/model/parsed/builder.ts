...
export class ModelBuilder {
    ...
    get currentState(): SimulatedType | null {
        return this.simulatedTypeStack.length > 0 ? this.simulatedTypeStack[this.simulatedTypeStack.length - 1] : null;
    }
    ...
    endDefinition(): void {
        ...
        assert(this.simulatedTypeStack.length === 1, 'endDefinition: expected exactly one type on stack');
    ...
    restoreState(state: BuilderStateSnapshot): void {
        ...
        this.simulatedTypeStack = this.simulatedTypeStack.slice(0, state.simulatedTypeStackLength);
    }
        ...
        while (ip < this.instructions.length) {
            ...
            switch (instruction.type) {
                ...
                case 'endDefinition':
                    ...
                    model.values.push(new Definition(currentDefinitionName!, type));
                ...
                case 'startMemberModification':
                    model.values.push(new MemberModification(instruction.params[0], []));
                    break;
                case 'addMemberDeletion':
                    {
                        const memberModification = model.values[model.values.length - 1];
                        assert(memberModification instanceof MemberModification);
                        memberModification.values.push(new MemberDeletion(instruction.params[0]));
                    }
                    break;
                case 'addMemberAddition':
                    {
                        ...
                        const memberModification = model.values[model.values.length - 1];
                        assert(memberModification instanceof MemberModification);
                        memberModification.values.push(new MemberAddition(type));
                    }
                    break;
                case 'createVoidType':
                ...
                case 'addProductTypeMember':
                    {
                        ...
                        productType.members.push(new ProductMember(instruction.params[0], type));
                    }
                    break;
                case 'startTupleType':
                ...
                case 'createNamedTypeReference':
                    typeStack.push(new NamedTypeReference(instruction.params[0]));
                    break;
            }
            ip++;
        }
